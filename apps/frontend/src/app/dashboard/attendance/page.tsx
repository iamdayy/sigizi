  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-1 bg-slate-900/60 border border-slate-800 rounded-2xl p-5 backdrop-blur-xl space-y-4">
          ...
          <div className="space-y-3">
            <div className="p-3 bg-slate-800/60 rounded-xl border border-slate-700/60">
              <span className="text-xs text-slate-400 font-medium">Petugas / Relawan:</span>
            ...
            <div className="p-3 bg-slate-800/60 rounded-xl border border-slate-700/60">
              ...
              <div className="text-xs text-slate-300 font-mono break-all">{locationStatus}</div>
            </div>
            ...
            {!todayAttendance ? (
              <div className="space-y-3 pt-2">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase mb-1">Catatan (Opsional)</label>
                  <input
                    type="text"
                    value={notes}
                ...
                <button
                  ...
                  className="w-full py-3 bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-500 hover:to-emerald-500 text-white font-bold rounded-xl shadow-lg shadow-teal-500/25 transition-all flex items-center justify-center gap-2"
                >
              ...
              <div className="space-y-3 pt-2">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase mb-1">Catatan Check-Out</label>
                  <input
                    type="text"
                    value={notes}
                    onChange={(e) => setNotes(e.target.value)}
                ...
                <button
                  ...
                  className="w-full py-3 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white font-bold rounded-xl shadow-lg shadow-amber-500/25 transition-all flex items-center justify-center gap-2"
                >
        ...
        {/* History Table */}
        <div className="md:col-span-2 bg-slate-900/60 border border-slate-800 rounded-2xl overflow-hidden backdrop-blur-xl">
          <div className="p-4 border-b border-slate-800 flex items-center justify-between">
            <h3 className="font-semibold text-slate-100 flex items-center gap-2">
              <Users className="w-4 h-4 text-blue-400" />
              Riwayat Presensi Seluruh Personel & Relawan SPPG
            </h3>
            <span className="text-xs text-slate-400">Terverifikasi GPS Satelit</span>
          </div>
                ...
                {attendanceList && attendanceList.length > 0 ? (
                  attendanceList.map((att) => (
                    <tr key={att.id} className="hover:bg-slate-800/30">
                      ...
                      <td className="p-3 font-semibold text-slate-200">{att.user?.full_name || 'Relawan Dapur'}</td>
                      <td className="p-3 text-xs text-slate-400">{att.user?.role || 'VOLUNTEER'}</td>
